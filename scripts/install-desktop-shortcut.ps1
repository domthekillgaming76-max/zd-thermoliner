...
$downloadLauncher = Join-Path $env:USERPROFILE 'Downloads\ZD-Thermoliner-ERP-Windows-1.0.4.exe'
...
if ($targetExe) {
  ...
  if (-not (Test-Path $batPath)) {
    Write-Host "[ERREUR] Launcher introuvable. Telechargez ZD-Thermoliner-ERP-Windows-1.0.4.exe"
    exit 1
  }
