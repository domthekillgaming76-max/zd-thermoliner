-- 068 — Reset RP : admin étendu, logs non bloquants, une seule signature RPC

-- Rôles admin élargis (administrator / administrateur)
...
AS $$
  SELECT EXISTS (
    ...
    WHERE id = p_user_id
      AND (
        role IN ('pdg', 'patron', 'admin', 'administrator', 'administrateur')
...
-- Supprimer l'ancienne surcharge 1 paramètre (évite les conflits PostgREST)
DROP FUNCTION IF EXISTS public.admin_reset_rp_economy(text);

-- Réappliquer la version 3 paramètres avec journaux non bloquants
...
BEGIN
  ...
  IF to_regclass('public.road_sheets') IS NOT NULL AND to_regclass('public.telemetry_jobs') IS NOT NULL THEN
    EXECUTE 'UPDATE public.road_sheets SET telemetry_job_id = NULL WHERE telemetry_job_id IS NOT NULL';
  END IF;
  ...
  IF to_regclass('public.company_stats') IS NOT NULL THEN
    EXECUTE $sql$
      UPDATE public.company_stats SET
        total_km = 0, total_deliveries = 0, total_revenue = 0,
        total_fuel_cost = 0, estimated_profit = 0,
        today_km = 0, today_deliveries = 0, today_revenue = 0,
        stats_date = CURRENT_DATE, updated_at = now()
    $sql$;
  ...
  IF to_regclass('public.drivers') IS NOT NULL THEN
    EXECUTE $sql$
      UPDATE public.drivers SET
        monthly_km = 0, total_km = 0, deliveries_count = 0,
        driving_hours_month = 0, eco_driving_score = 0, rest_hours_month = 0,
        driver_rating = 0, salary_base = 0,
        driving_status = 'resting', presence_status = 'offline',
        updated_at = now()
    $sql$;
  END IF;
  ...
  IF to_regclass('public.driver_stats') IS NOT NULL THEN
    EXECUTE $sql$
      UPDATE public.driver_stats SET
        total_distance = 0, total_deliveries = 0, total_earnings = 0,
        total_fuel = 0, total_tolls = 0, monthly_distance = 0,
        monthly_deliveries = 0, monthly_salary = 0, monthly_net_profit = 0,
        reputation = 0, last_delivery_date = NULL, updated_at = now()
    $sql$;
  END IF;

  IF to_regclass('public.trucks') IS NOT NULL THEN
    EXECUTE $sql$
      UPDATE public.trucks SET mileage = 0, updated_at = now()
  ...
  EXCEPTION WHEN OTHERS THEN
    v_log_err := SQLERRM;
    v_deleted := v_deleted || jsonb_build_object('ensure_driver_warning', v_log_err);
  END;
  ...
  BEGIN
    ...
    VALUES (
      v_actor, 'rp_reset',
      jsonb_build_object(
        'deleted', v_deleted, 'confirmation', 'RESET RP',
        'delete_wall_posts', p_delete_wall_posts,
        'delete_notifications', p_delete_notifications
      )
    );
  EXCEPTION WHEN OTHERS THEN
    v_deleted := v_deleted || jsonb_build_object('admin_actions_log_skipped', SQLERRM);
  END;
  ...
  EXCEPTION WHEN OTHERS THEN
    v_deleted := v_deleted || jsonb_build_object('security_log_skipped', SQLERRM);
  END;
